...
public class WorldGenerator {
    ...
    private void generateBuildings() {
        buildingService.createBuilding(BuildingType.WAREHOUSE, new Coordinates((World.SIZE_WIDTH / 2) - 2, (World.SIZE_HEIGHT / 2) - 2, 3));
    ...
    private void generateUnits() {
        ...
        for (int i = 0; i < WorldParameters.PEASANTS; i++) {
            unitService.createUnit(UnitType.PEASANT, positionUtil.getCoordinatesNear(warehouse, 1, 4));
        }

        for (int i = 0; i < WorldParameters.WORKERS; i++) {
            unitService.createUnit(UnitType.WORKER, positionUtil.getCoordinatesNear(warehouse, 1, 4));
        }
    }
}
